...
Реализация проекта позволит добиться значительного повышения надежности и отказоустойчивости сетевой инфраструктуры за счёт децентрализации и динамической маршрутизации. Также будут достигнуты цели по оптимизации и стабилизации задержек RTT как внутри одного дата-центра, так и при межцентровом взаимодействии, что положительно скажется на качестве обслуживания и отклике сервисов.

Дополнительно, за счёт преимуществ масштабируемости EVPN/VXLAN, удастся обеспечить рост пропускной способности и упростить процесс расширения сети без значительных изменений существующей архитектуры. Это делает предложенное решение актуальным как для текущих задач, так и для будущего развития инфраструктуры.
